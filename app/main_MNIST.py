...
def main_mnist(num_train , num_test, embedding_method, B=3000, block_size=[5, 5]):
    ...
    model.add_layer(layers.KIMLayer(block_size=block_size[0], channels_next = 6, stride = 1, padding=True, emb=embedding_method, num_blocks=B))
    model.add_layer(layers.MaxPoolingLayer(pool_size=2))
    model.add_layer(layers.KIMLayer(block_size=block_size[1], channels_next = 16, stride = 1, padding=False, emb=embedding_method, num_blocks=B))
    model.add_layer(layers.MaxPoolingLayer(pool_size=2))
    #model.add_layer(layers.KIMLayer(block_size=block_size[2], channels_next = 30, stride = 1, padding=True, emb=embedding_method, num_blocks=B))
    #model.add_layer(layers.MaxPoolingLayer(pool_size=2))
