...
conv_block_pair = []
compressed_blocks = []
block_outputs = block_outputs[0]

for n in range(block_outputs.shape[0]):   
    for i in range(block_outputs.shape[1]):
        for j in range(block_outputs.shape[2]):
            compressed_blocks.append(block_outputs[n, i, j])
