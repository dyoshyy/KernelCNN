...
def main(num_train, num_test, num_epoch):
    ...
    batch_size = 512
    epochs = num_epoch
    es = EarlyStopping(monitor='val_loss', mode='auto', patience=7, verbose=0)
    checkpoint = ModelCheckpoint("./weights/model_weights_epoch_{epoch:02d}.h5", save_weights_only=True, period=10)
    model.fit(train_X, train_Y, batch_size=batch_size,epochs=epochs, callbacks=[es, checkpoint], validation_split=0.2)
    ...
    if True:
        for epoch in epochs_to_check:
            ...
            visualize_emb(compressed_blocks[:, 4:6], sampled_blocks, block_labels, f"LeNet_epoch{epoch}", 5, 5, 6)
...
es = EarlyStopping(monitor='val_loss', mode='auto', patience=7, verbose=0)
checkpoint = ModelCheckpoint("./weights/model_weights_epoch_{epoch:02d}.h5", save_weights_only=True, period=10)
model.fit(train_X, train_Y, batch_size=batch_size,verbose=0, epochs=epochs, callbacks=[es, checkpoint], validation_split=0.2)
