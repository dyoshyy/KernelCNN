...
class KSLE_ML:
    ...
    def _change_kernel(self, kernel, kopt1=None, setonly=False):
        ...
        if setonly == False:
            ...
            print(
                "(change)self.C.shape = " + str(self.C.shape) + " rank test=" + str(ans)
            )
        return ans
    ...
    def explicit_map(self, Y):
        ...
        k = self._calc_kernel(
            self.X, Y
        )  # innerproduct to the training sample via kernel
        Z = np.dot(C.T, k)
        return Z
    ...
    def get_parameter(self):  # depends on the kernel to be used
        ...
        alpha = (
            np.sum(np.sum(np.power(distances, 2), axis=1) / self.n_neighbors)
            / self.X.shape[0]
        )
        return alpha
    ...
    def fit(
        ...
        Z = Z[
            :, 1 : m_dim + 1
        ]  # This is for 2D display. Increase 3 to any number for dimension reduction
...
def SLE(
    X, Y, la=0.3, map_d=2, n_neighbors=None
):  # defaul value of lambda is 0.3 to 0.4; no kernel is necessary for SLE
    ...
    if n_neighbors == None:
        n_neighbors = int(
            1.5 * n_samples / n_classes
        )  # 1.5 times average sample number / class
