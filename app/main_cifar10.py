...
def main_cifar10(num_train , num_test, embedding_method, num_blocks):
    ...
    model.add_layer(layers.KIMLayer(block_size=5, channels_next = 6, stride = 1, emb=embedding_method, num_blocks=num_blocks))
    model.add_layer(layers.AvgPoolingLayer(pool_size=2))
    model.add_layer(layers.KIMLayer(block_size=5, channels_next = 16, stride = 1, emb=embedding_method, num_blocks=num_blocks))
    model.add_layer(layers.AvgPoolingLayer(pool_size=2))
